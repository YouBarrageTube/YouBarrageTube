...
exports.insertComment = function (videoUrl, comment, date) {
    MongoClient.connect(url, function (err, db) {
        if (err) throw err;
        var dbo = db.db(dbName);
        var query = {_id: videoUrl};
        var newvalues = {$push: {comments: {comment: comment, date: date}}};
        ...
        dbo.collection(collectionName).find(query).project(options).toArray(function (err, result) {
            ...
            if (result.length){
                ...
                comments.sort(function (a,b) {
                    return parseFloat(a)-parseFloat(b);//TODO: change to date parsing
                });
